...
2. Run the generator from the repo root:
   ```bash
python scripts/cpp/generate_snapshot_schema.py      --output GeneralsMD/Code/Tools/GameStatePipeWriter/GeneratedSnapshotSchema.h      --source-root GeneralsMD/Code      --source-root Core/GameEngine      --compile-commands build/compile_commands.json      --gamestate GeneralsMD/Code/GameEngine/Source/Common/System/SaveGame/GameState.cpp
   ```
