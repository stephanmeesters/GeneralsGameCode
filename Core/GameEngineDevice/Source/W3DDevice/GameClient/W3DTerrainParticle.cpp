...
Bool isTerrainFlat(WorldHeightMap& map, const IRegion2D& bounds)
{
	...
	const UnsignedByte height = firstRow[0];
...
void W3DTerrainParticle::drawRegion(WorldHeightMap& map, const ParticleContext& particle, const IRegion2D& bounds)
{
	if (bounds.width() == 2 && bounds.height() == 2 || isTerrainFlat(map, bounds))
	{
		...
		drawQuad(map, particle, bounds, flipped);
		return;
	}
...
void W3DTerrainParticle::drawQuad(WorldHeightMap& map, const ParticleContext& particle, const IRegion2D& bounds, Bool flipped)
{
	...
	const UnsignedShort a = addVertex(map, particle, bounds.lo.x, bounds.lo.y);
	const UnsignedShort b = addVertex(map, particle, bounds.hi.x - 1, bounds.lo.y);
	const UnsignedShort c = addVertex(map, particle, bounds.lo.x, bounds.hi.y - 1);
	const UnsignedShort d = addVertex(map, particle, bounds.hi.x - 1, bounds.hi.y - 1);
	if (flipped)
	{
		addTriangle(b, c, a);
		addTriangle(b, d, c);
