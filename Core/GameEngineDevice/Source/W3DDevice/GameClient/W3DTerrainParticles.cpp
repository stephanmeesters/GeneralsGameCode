...
IRegion2D W3DTerrainParticles::calcTerrainBounds(WorldHeightMap& map, const Vector3& loc, Real projectedRadius) const
{
	...
	bounds.intersectWith(map.getLogicalBounds());
	bounds.intersectWith(m_terrainInViewBounds);
	...
	Int activePointCount)
{
	...
	m_pointSize = sizes;
	m_pointOrientation = orientations;
...
void W3DTerrainParticles::setBoundingBox(const AABoxClass& worldBoundingBox)
{
	...
	m_terrainInViewBounds.hi.y = REAL_TO_INT_CEIL((worldBoundingBox.Center.Y + worldBoundingBox.Extent.Y) / MAP_XY_FACTOR) + 1;
}
